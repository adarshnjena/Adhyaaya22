...
let page_text = "Please wait"
    onMount(async () => {
        ...
        if (verify.order_status == "PAID") {
            page_text = "Payment has been sucessfully recieved. You may return to dashboard."
            
        } else {
            page_text = "Payment has not be successfully recieved. Please try again. Deductions, if any, will be refunded."
        }
    })
</script>
